...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('artsInDialogue.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('artsInDialogue.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('artsInDialogue.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
